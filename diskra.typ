...
#align(center)[#image("dp/10.png", width: 70%)]
...
#def("Обобщенная композиция соответствий")[
  *Обобщенной композицией соответствий* называют:
  $ rho circle sigma = {(x, y) : x in A, y in D, exists z in B inter C : (x, z) in rho " и " (z, y) in sigma} $
]
...
#def("Маршрут")[
  Маршрутом в графе называется чередующаяся последовательность вершин и инцидентных им ребер вида $v_1, e_1, v_2, e_2, ..., e_n, v_{n+1}$, где вершина $v_1$ является началом маршрута, а $v_{n+1}$ — его концом.
]

*Длиной маршрута* считается число содержащихся в нем ребер. Сокращенно маршрут можно обозначать только последовательностью его вершин: $P=v_1…v_{n+1}$.

#def("Цепь")[
  *Цепью* называется маршрут, в котором все составляющие его ребра различны (т.е. каждое ребро встречается не более одного раза).

  *Цепь называется простой*, если в ней попарно различны не только ребра, но и все вершины, кроме, быть может, первой и последней.
]

#def("Цикл")[
  *Циклом* называют любую замкнутую цепь.

  *Замкнутая простая цепь* называется простым циклом.
  Простой цикл, имеющий длину $n$, также принято называть $n$-угольником.
]

#def("Путь")[
  *Путь* — это последовательность вершин $v_0, v_1, ..., v_n$, в которой для любой пары соседних вершин $v_i$ и $v_{i+1}$ существует дуга, ведущая из $v_i$ в $v_{i+1}$.
  Длина пути определяется количеством его дуг.
]

#def("Контур")[
  *Контуром* называют простой путь ненулевой длины, у которого начальная и конечная вершины совпадают.

  *Ориентированный граф, не содержащий контуров*, называется бесконтурным (или ациклическим).
]

#def("Транзитивное замыкание")[
  Бинарное отношение $rho$ называется транзитивным, если из выполнения условий $a rho b$ и $b rho c$ всегда следует $a rho c$. На языке графов это означает, что любая двухзвенная направленная ломаная должна замыкаться прямой стрелкой из ее начала в конец. \
  *Транзитивное замыкание бинарного отношения* — это процедура дополнения графа новыми дугами так, чтобы он стал транзитивным.

  Согласно теореме, чтобы получить транзитивное замыкание на графе, надо замкнуть все направленные ломаные линии (пути) этого орграфа. \
  Матрицу достижимости графа можно рассматривать как матрицу транзитивного (и рефлексивного) замыкания бинарного отношения непосредственной достижимости. Нахождение транзитивного замыкания реализуется, например, *алгоритмом Уоршелла*.
]

#def("Прямое транзитивное замыкание")[
  *Прямое отображение* $Gamma_(x_i)$ — множество вершин, в которые существует путь из $x_i$ длиной 1 ребро. \
  *Прямым транзитивным замыканием* $hat(Gamma)_(x_i)$ называют множество вершин, в которые существует путь из $x_i$ (множество вершин, достижимых из $x_i$):
  $ hat(Gamma)_(x_i) = {x_i} union Gamma_(x_i) union Gamma_(x_i)^2 union ... union Gamma_(x_i)^n $

  Прямое транзитивное замыкание строится напрямую по дугам исходного орграфа и отражает отношение достижимости «вперед». Геометрически это означает, что если в графе существует какой-либо направленный путь из вершины $u$ в вершину $v$, то в прямом транзитивном замыкании обязательно появится прямая дуга из $u$ в $v$.
]
...
#def("Обратное транзитивное замыкание")[
  ...
  *Обратным транзитивным замыканием* $hat(Gamma)_(x_i)^(-1)$ называют множество вершин, из которых существует путь в $x_i$ (множество вершин, из которых достижима $x_i$):
  $ hat(Gamma)_(x_i)^(-1) = {x_i} union Gamma_(x_i)^(-1) union Gamma_(x_i)^(-2) union ... union Gamma_(x_i)^(-n) $

  Обратное транзитивное замыкание используется для определения обратной достижимости (то есть поиска всех вершин, из которых можно попасть в заданную). Для его получения в исходном графе сначала меняют направления всех стрелок на противоположные (получают обратное отношение), а затем уже для полученного перевернутого графа алгоритмически строят транзитивное замыкание.
]
...
Если в орграфе любая вершина достижима из любой вершины, с учетом направлений ребер, то такой граф называется *сильно связным*.
$ forall x_i in X : hat(Gamma)_(x_i) = X $
...
Часть $G_1$ графа $G$ ($G_1 subset.eq G$) называется *компонентой сильной связности* графа $G$, если не существует другого сильно связного подграфа $G_2 subset G$, $G_2 != G_1$, такого что $G_1 subset G_2$.

#thm("")[
  Компоненты сильной связности образуют разбиение множества вершин и это разбиение единственное.
]

*Алгоритм Мальгранжа:*
Алгоритм Мальгранжа применяется для разложения ориентированного графа на непересекающиеся компоненты сильной связности.
В основе алгоритма лежит идея о том, что класс эквивалентности (бикомпонента) $C_(x_i)$ для любой вершины $x_i$ может быть найден как пересечение двух множеств:

- $hat(Gamma)_(x_i)$ — множества всех вершин, достижимых из $x_i$ (прямое транзитивное замыкание);
- $hat(Gamma)_(x_i)^(-1)$ — множества всех вершин, из которых достижима $x_i$ (обратное транзитивное замыкание). Формула пересечения: $ hat(Gamma)_(x_i) inter hat(Gamma)_(x_i)^(-1) = C_(x_i) $

Шаги:
1. Для произвольной $x_i$ найти $C_(x_i)$ по формуле $C_(x_i) = hat(Gamma)_(x_i) inter hat(Gamma)_(x_i)^(-1)$
...
#thm("о точке сочленения")[
  $x_k$ — является точкой сочленения связного графа $G(X, U)$ тогда и только тогда, когда существуют различные вершины $x_i, x_j in X, x_i != x_j != x_k$ такие что любой путь (цепь для неографов), соединяющий $x_i$ и $x_j$ проходит через $x_k$.
]

#align(center)[#image("dp/37.png", width: 60%)]
...
- $N_0 = {x_i in X, Gamma^(-1)_(x_i) = emptyset}$ — вершины, не имеющие входящих ребер;
- $N_1 = {x_i in X without N_0, Gamma^(-1)_(x_i) subset N_0}$ — вершины, в которые входят ребра только из $N_0$;
- $N_2 = {x_i in X without (N_0 union N_1), Gamma^(-1)_(x_i) subset N_0 union N_1}$ — вершины, в которые входят ребра только из $N_0$ или $N_1$;
- $dots$
- $N_m = {x_i in X without (N_0 union dots union N_(m-1)), Gamma^(-1)_(x_i) subset N_0 union dots union N_(m-1)}$ — вершины, в которые входят ребра только из $N_0, dots, N_(m-1)$;

#def("Порядковая функция")[
  *Порядковой функцией* $O$ орграфа без контуров и петель называется функция, задающая отношение порядка на множестве ее вершин, областью значений которой является ${0, 1, ..., m}$, причем если некоторый $x_i$ входит в уровень $N_k, k in {0, 1, ..., m}$, то $O(x_i) = k$.
]
...
#align(center)[#image("dp/page46_img0.png", width: 70%)]

*Алгоритм Демукрона*

Граф задается матрицей смежности $M$; $k = 0$;
+ Строим строку $Lambda_k$ под матрицей $M$, где $Lambda_k(x_i)$ — количество входящих ребер.
+ Проверяем, существует ли хотя бы одна такая вершина $x_epsilon : Lambda_k(x_epsilon) = 0$?
  - *Да:*
...
#align(center)[#image("dp/page47_img0.png", width: 70%)]

Если поменять $Gamma^(-1)_(x_i)$ на $Gamma_(x_i)$ в определении, то алгоритм пойдет "вправо" от матрицы. Получим граф, в котором столько же уровней, но направление — обратное.

#alert[
  Если порядковая функция строится по прямым / обратным ребрам, число уровней не изменяется, а состав уровней — не обязательно сохранится.
]
...
#def("Эйлеров цикл")[
  *Эйлеровым циклом* (обходом) в неографе называют обход (цикл), включающий все ребра графа в точности один раз.
]

#thm("Теорема Эйлера")[
  Произвольный связный неограф является Эйлеровым (содержит Эйлеров цикл) тогда и только тогда, когда все его вершины имеют четную степень.
]
...
_Рекомендации к алгоритму:_ нужно стараться не использовать ребра, являющиеся мостами в непомеченной части графа, до тех пор, пока есть другой выбор.


== 41. Гамильтоновы графы. Теорема Оре о гамильтоновом цикле в связном неографе.

#def("Гамильтонов цикл")[
  *Гамильтоновым обходом* (циклом) в неографе называют цикл (обход), содержащий все вершины графа, каждую в точности один раз. Граф, содержащий Гамильтонов цикл, называется *Гамильтоновым*.
]
...
#thm("Теорема Оре")[
  Дан неограф $G$ порядка $n$, где $n >= 3$. Если для любой пары несмежных вершин $(x_i, x_j)$ выполняется неравенство $p(x_i) + p(x_j) >= n$, то $G$ — Гамильтонов граф.
]


== 42. Эйлеровость и гамильтоновость в орграфах.

*Эйлеровость в орграфах*
Если орграф $G$ — эйлеров граф, то:
...
#thm("Открытый эйлеров путь")[
  Связный орграф $G$ содержит открытый эйлеров путь тогда и только тогда, когда в нем есть две вершины $x, y$, такие что $p^(-)(x) = p^(+)(x) + 1$, $p^(-)(y) = p^(+)(y) - 1$, а для любой оставшейся вершины $z in X without {x, y}$ верно, что $p^(-)(z) = p^(+)(z)$.
]

*Гамильтоновость в орграфах*
#thm("Достаточное условие Гамильтоновости в орграфе")[
  Дан орграф порядка $n = |X| >= 2$. Если для любой пары различных несмежных вершин $x_i, x_j$ выполняется неравенство $p(x_i) + p(x_j) >= 2n - 1$, то граф $G$ содержит Гамильтонов контур.
]

#alert[
  Условие достаточное, поэтому, даже если оно не выполняется, в графе может быть Гамильтонов контур.
]
...
#align(center)[#image("dp/page50_img2.png", width: 40%)]

#def("Двудольный граф")[
  Граф называется *двудольным* (графом Кёнига), если множество его вершин можно разбить на два непересекающихся подмножества $X_1$ и $X_2$ ($X_1 inter X_2 = emptyset$), таких что для каждого ребра верно $forall(x_i, x_j) : x_i in X_1, x_j in X_2$. $X_1$ и $X_2$ — доли двудольного графа.
]
...
#align(center)[#image("dp/page51_img0.png", width: 50%)]
#align(center)[#image("dp/page51_img1.png", width: 30%)]
...
#thm("Теорема Эйлера")[
  Для связного плоского графа с $n$ вершинами, $m$ ребрами и $r$ гранями справедливо равенство:
  $ n - m + r = 2 $
]

*Следствия из теоремы Эйлера:*
1. Если число вершин связного планарного графа больше 3, то число его ребер удовлетворяет неравенству $m <= 3n - 6$.
2. В каждом планарном графе существует вершина, степень которой не больше 5.

#thm("Теорема о пяти красках")[
  Всякий планарный граф можно раскрасить 5-ю цветами (так, чтобы любые две смежные вершины имели разные цвета).
]


== 45. Гомеоморфизм графов. Теорема Понтрягина–Куратовского о планарном графе. Искаженность и толщина графа.

#def("Гомеоморфизм графов")[
  Два графа называют *гомеоморфными*, если они изоморфны или могут быть приведены к изоморфным путем конечного числа слияний и разбиений их ребер (добавления или удаления вершин степени 2 на ребрах).
]

#thm("Теорема Понтрягина-Куратовского")[
  Граф планарен тогда и только тогда, когда он не содержит подграфов, гомеоморфных полному графу $K_5$ и полному двудольному графу $K_(3,3)$.
]

#align(center)[#image("dp/page52_img0.png", width: 70%)]
...
2. Если $x_i, x_j in X, x_i != x_j$, то эти две вершины соединяет единственная простая цепь. (Существование цепи следует из связности, а единственность — из отсутствия циклов).
3. Если две несмежные вершины соединить ребром, то введение этого ребра даст граф, содержащий ровно один цикл.
4. Любое произвольное дерево (при $n >= 2$) содержит по крайней мере две концевые вершины (листья), имеющие степень 1.
5. *Теорема Кейли:* число деревьев, которые можно построить на $n$ различных вершинах, равно $n^(n-2)$.

*Ориентированные деревья*
Орграф $G$ называется ориентированным деревом (ордеревом), если выполняются следующие условия:
1. Существует ровно одна вершина, не имеющая предшествующих, т.е. $p^(+)(x_i) = 0$. Эта вершина называется *корнем* дерева.
...
*Бинарные деревья*
Дерево называется *бинарным*, если при представлении его ордеревом полустепень исхода каждой вершины не превышает 2: $forall x_i in X : p^(-)(x_i) <= 2$.
Если полустепень исхода каждой вершины, отличной от листа, строго равна 2, то ордерево называется *полным бинарным ордеревом* (все листья полного бинарного ордерева располагаются в одном ярусе).

#thm("Теорема о высоте бинарного дерева")[
  Бинарное дерево с $n$ листьями имеет высоту, не меньшую, чем $log_2 n$.
]
...
#def("Остовный подграф")[
  Часть графа $G_1(X_1, U_1)$ от графа $G(X, U)$ называется *остовным подграфом* (суграфом), если $X_1 = X, U_1 subset.eq U$.
  Если остовной подграф является деревом (связен и без циклов), его называют *остовным деревом* (или просто *остовом*).
]

#thm("О числе удаляемых ребер")[
  Число ребер произвольного неографа $G$, которое необходимо удалить для получения остова, не зависит от порядка их удаления и равно $m - n + k$, где $m = |U|, n = |X|, k$ — количество компонент связности в графе $G$.
]

- $nu(G) = m - n + k$ — *цикломатическое число* графа (циклический ранг графа).
- $nu^*(G) = n - k$ — *коциклический ранг* графа $G$ (коранг) — количество ребер остова.
Отсюда: $nu(G) + nu^*(G) = m$.
...
3. множество вершин $X$ включает в себя $X_0$ ($X_0 subset.eq X$);
...
Пусть $G(X, U, Omega)$ — связный неориентированный граф, в котором каждому ребру $u_i$ сопоставлен вес $omega_i in Omega$, причем $|Omega| = |U|$. Требуется построить остов графа $G$ минимального (или максимального) веса.

*Алгоритм Прима*
Алгоритм основан на пошаговом построении остова путем добавления к уже построенной части такого ребра, которое имеет минимальный вес среди всех ребер, соединяющих построенную часть с оставшимися вершинами.

Пусть множество вершин $X$ можно разбить на два непересекающихся подмножества $X_1$ и $X_2$ ($X = X_1 union X_2, X_1 inter X_2 = emptyset$).
Введем понятие пошагового расстояния между этими подмножествами:
$ d(X_1, X_2) = min{omega(x_i, x_j) : x_i in X_1, x_j in X_2} $

*Шаг 1: Присвоение начальных значений*
Полагаем $X_1 = {x_1}$ (произвольная стартовая вершина), $X_2 = X without X_1$.
$U' = emptyset$ — множество ребер искомого остова.

*Шаг 2: Обновление данных*
Находим ребро $(x_i, x_j)$, где $x_i in X_1, x_j in X_2$, такое, что $omega(x_i, x_j) = d(X_1, X_2)$.
Полагаем:
$ X_1 &= X_1 union {x_j} \
  X_2 &= X_2 without {x_j} \
  U' &= U' union {(x_i, x_j)} $
...
Если $X_1 = X$ и $X_2 = emptyset$, алгоритм завершен. $G'(X_1, U')$ — искомый минимальный остов графа $G$. Иначе возвращаемся к шагу 2.

#alert[
  Если требуется получить остов максимального веса, то функция $min$ в определении пошагового расстояния заменяется на $max$.
]

#align(center)[#image("dp/page55_img0.png", width: 60%)]
#align(center)[#image("dp/page55_img1.png", width: 60%)]
...
Найти в графе $G$ пути, соединяющие вершины $s$ и $t$ и доставляющие наименьший (или наибольший) значения некоторой аддитивной функции, определенной на ребрах графа. В качестве такой функции рассмотрим весовую функцию $omega(x_i, x_j)$, ставящую в соответствие каждому ребру его длину (вес).
Длина пути — это сумма длин ребер, его составляющих.
...
*Отыскание кратчайшего пути в невзвешенном графе (волновой алгоритм / поиск в ширину):*
В невзвешенном графе длина любого ребра принимается равной 1. Кратчайшим путем будет путь с минимальным числом ребер.
1. Вершину $s$ помечаем меткой 0: $mu(s) = 0$.
2. Находим $Gamma_s$ (множество смежных с $s$ вершин). Все непомеченные вершины $x_i in Gamma_s$ помечаем меткой 1.
3. Находим $Gamma_1 = union_(x in Gamma_s) Gamma_x$, после чего все непомеченные вершины $x_i in Gamma_1$ помечаем меткой 2.
...
Алгоритм Дейкстры используется для графов с неотрицательными весами ребер.
Метки, присваиваемые вершинам, бывают двух типов:
...
1. *Инициализация:* $mu(s) = 0^*$, для всех остальных вершин $forall x in X without {s}: mu(x) = oo$. Текущая вершина $x_T = s$.
...
2. Среди смежных вершин $x_i in Gamma^(-1)_(x_T)$ ищем ту, для которой выполняется условие оптимальности:
...
Алгоритм Беллмана-Форда позволяет находить кратчайшие пути даже в графах с отрицательными весами ребер (при отсутствии контуров отрицательного веса).
Матрицу смежности (весов) предварительно дополняем столбцом меток вершин и столбцом отметок о просмотре строк.

*Этап 1: Нахождение длин кратчайших путей*
1. *Начальные метки:* $mu(s) = 0$, для всех $x in X without {s} : mu(x) = oo$.
2. Просматриваем любую непросмотренную строку матрицы (например, сверху вниз). Пусть это строка вершины $x_i$.
   Проверяем для каждого столбца $x_j$: если $mu(x_i) + omega(x_i, x_j) < mu(x_j)$, то обновляем метку:
...
#def("Транспортная сеть")[
  *Транспортной сетью* называют связный ориентированный взвешенный граф $G(X, U, C)$, для которого выполняются условия:
  - граф не содержит петель;
  - ровно одна вершина является источником ($s$) и ровно одна — стоком ($t$);
  - каждому ребру $(x_i, x_j) in U$ сопоставлено число $c(x_i, x_j) >= 0$ — *пропускная способность* ребра.
]

На множестве дуг $U$ определена функция $phi(x_i, x_j)$, называемая *потоком*, которая удовлетворяет следующим условиям:
1. *Ограничение пропускной способности:* $0 <= phi(x_i, x_j) <= c(x_i, x_j)$.
2. *Условие сохранения потока (закон Кирхгофа) для всех промежуточных узлов:*
   Для любой вершины $x_i != s, t$, сумма входящих потоков равна сумме исходящих:
   $ sum_(x_k in Gamma^(-1)_(x_i)) phi(x_k, x_i) = sum_(x_j in Gamma_(x_i)) phi(x_i, x_j) $

- *Остаточной пропускной способностью* ребра называется разность $delta(x_i, x_j) = c(x_i, x_j) - phi(x_i, x_j)$.
...
Определить функцию потока $phi(x_i, x_j)$ на всех ребрах сети так, чтобы суммарная величина потока, доставляемого из источника $s$ в сток $t$, была максимально возможной.
...
*Увеличивающий маршрут*
Маршрут из $s$ в $t$, на котором прямые (ориентированные по ходу маршрута) ребра не насыщены, а обратные (ориентированные против хода) имеют строго положительный поток, называют *увеличивающим маршрутом*.

- *Теорема 1:* Если найден путь из $s$ в $t$, где все ребра прямые и не насыщены, поток вдоль этого пути можно увеличить на величину $epsilon^* = min(delta(x_i, x_j))$ по всем ребрам пути.
- *Теорема 2:* Если найден увеличивающий маршрут, на котором есть обратные ребра, поток можно увеличить на $epsilon^* = min(delta^*, phi^*)$, где $delta^*$ — минимум остаточных пропускных способностей прямых ребер, а $phi^*$ — минимум текущих потоков на обратных ребрах. При этом поток по прямым ребрам увеличивается на $epsilon^*$, а по обратным — уменьшается на $epsilon^*$.
- *Теорема 3:* Поток в сети максимален тогда и только тогда, когда в ней не существует увеличивающего маршрута.

*Алгоритм Форда-Фалкерсона:*
1. Назначить начальный допустимый поток (обычно нулевой: $phi(x_i, x_j) = 0$).
2. Выполнить поиск увеличивающего маршрута (например, методом расстановки меток).
3. Если маршрут найден, скорректировать потоки вдоль него на величину $epsilon^*$ (увеличить на прямых дугах, уменьшить на обратных). Пересчитать остаточные пропускные способности.
