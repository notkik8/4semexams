...
*Процессор i8086* – родоначальник семейства процессоров Intel x86, который определил особенности, присущие процессорам этого ряда.

РОНы/Данные/Адрес: 16/16/20; Осн. хар-ки: 4-8 МГц, 1 Мб

#align(center)[*Упрощенная структурная схема процессора i8086:*]

#align(center)[#image("mp/1.png")]

В общем случае процессор состоит из двух модулей – модуль выполнения и модуль интерфейса, а их взаимодействие происходит через шину данных.

В его состав входят: *устройство управления* (УУ), *арифметико-логическое устройство* (АЛУ), *блок преобразования* (формирования) *адресов* и *регистры*:

  - *УУ* дешифрует коды команд и формирует необходимые управляющие сигналы.
  - *АЛУ* осуществляет необходимые арифметические и логические преобразования данных.
  - В блоке *преобразования* адресов формируются физические адреса данных, расположенных в основной памяти.
  - *Регистры* – внутренняя память процессора – используются для хранения управляющей информации (в основном адресов и данных).
...
1. *Четыре регистра общего назначения* (называемые также регистрами данных):
  - *AX* (Accumulator) – регистр-аккумулятор (для умножения, деления (записывается целая часть), для команд ввода/вывода);
  - *BX* (Base) – базовый регистр;
  - *СХ* (Counter) – счетчик (для циклов);
  - *DX* (Data) – регистр-расширитель аккумулятора (для остатка от деления, адрес ввода/вывода);
2. *Три адресных регистра*, которые должны использоваться для хранения частей адреса данных или применяется соответствующая команда:
  - *SI* (Source Index) – регистр индекса источника;
  - *DI* (Destination Index) – регистр индекса приемника;
  - *BP* (Base Pointer) – регистр-указатель базы стека;
3. *Три управляющих регистра:*
  - *SP* (Stack Pointer) – регистр-указатель стека;
  - *IP* (Instruction Pointer) – регистр-счетчик команд;
  - *Flags* – регистр флагов;
4. *Четыре сегментных регистра:*
  - *CS* (Code Segment) – регистр сегмента кодов;
  - *DS* (Data Segment) – регистр сегмента данных;
  - *ES* (Extra Segment) – регистр дополнительного сегмента данных;
...
#alert()[ Адресовать отдельно бит нельзя. Если необходимо получить доступ к определенному биту, то сначала ищется соответствующий байт, а затем уже в нем – нужный бит.]

В i8086 для адресации оперативной памяти используются адресная шина размером *20 бит*, осуществляется работа с памятью до *1 МБ*. Каждый сегментный регистр используется для хранения памяти. Адрес, задаваемый в регистре или команде, представлял собой не физический адрес памяти, а смещение в некоем сегменте относительно начального (базового) адреса этого сегмента. Начальные адреса всех используемых в данный момент сегментов задаются процессору в специальных сегментных регистрах. 


Процессор i8086 содержит 16-ти разрядную модель памяти.

#align(center)[*Схема адресации «база + смещение»:*$ A_ф = A_б + А$]
